...
// something is actually wrong. There is no matching budget for reading the
// track list any more: that runs in-process through server/spotify.js, where
// every request carries its own timeout instead of needing a watchdog.
...
async function main() {
  ...
  try {
    ...
    // Read the track list over the Spotify Web API and write the save file
    // ourselves, rather than shelling out to `spotdl save`.
    //
    // `spotdl save` opens ~50 concurrent connections and sets no socket read
    // timeout, so one stalled response hangs it permanently. Measured on this
    // playlist: 474s of silence and no save file, reproducing on a 10-track
    // album with known-good credentials - so neither quota nor credentials.
    // The same playlist over the paginated endpoint: 116 tracks, 2 requests,
    // 1.41 seconds.
    //
    // There is deliberately no fallback to `spotdl save`. Falling back to a
    // path that hangs forever would convert a clear failure into the exact
    // silence this replaces.
    let tracks;
    try {
      tracks = await resolveTracks(job.url, {
        onProgress: ({ done, total }) => {
          // This phase used to be an unbroken multi-minute silence. It now
          // reports real numbers while it works.
          latestLine = total
            ? `Reading track list: ${done} of ${total}`
            : `Reading track list: ${done}`;
          mergeJob(jobId, { metadataDone: done, metadataTotal: total ?? null }).catch(
            () => {},
          );
        },
      });
    } catch (err) {
      // SpotifyError messages are already written for a non-technical reader.
      if (err instanceof SpotifyError) throw new Error(err.message);
      throw err;
    }

    if (isCancelled()) return finish("cancelled");

    // The download phase consumes this file, so its shape is spotDL's `Song`
    // dataclass. scripts/schema-test.mjs pins the required keys.
    await writeFile(saveFile, JSON.stringify(tracks), "utf8");

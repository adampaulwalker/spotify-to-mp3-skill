...
| C10 | Download throughput | **OPEN** | Measured ~150s per track, twice (412s for 2 tracks direct, 300s for 2 via the extension). At that rate a 116-track playlist takes roughly five hours. Metadata is no longer the bottleneck; this is the yt-dlp/source phase. Not yet isolated between the day's flaky network, YouTube throttling, and the engine's own settings |

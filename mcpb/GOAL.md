...
| C10 | Download throughput | **OPEN - being measured** | Two samples of 2 tracks took 412s and 300s. The "~150s per track, so ~5 hours for 116 tracks" reading of that was **wrong** and is retracted: `--threads 4` is already set, so tracks run concurrently and wall clock is not the sum of per-track times. Both samples were also 2-track runs where one track exhausted all three sources plus three retries before failing, which plausibly dominated the clock. A clean 12-track album measurement is running to replace the guess with a number |
